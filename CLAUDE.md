...
## 비동기 클라우드 에이전트(Jules)에 위임할 때 — **먼저 브랜치를 확인하라**

`jules` CLI 로 이 저장소를 위임하려 할 때 실물로 막힌 것들이다(2026-09-10 실측).

- ⛔⛔ **`jules remote new` 에 브랜치 플래그가 없다**(`--repo`·`--session`·`--parallel` 뿐).
  기본 브랜치(`main`)를 클론한다. 그런데 **작업 브랜치는 `main` 보다 한참 앞서 있고
  `main` 에 «없는 파일»이 있다** — 그 자리를 고치라고 위임하면 성립하지 않는다.
  ⇒ **던지기 전에 세라**(수를 여기 적지 않는다 — 미는 순간 낡는다):
  ```bash
  git rev-list --count origin/main..HEAD
  git show origin/main:<위임할 파일> >/dev/null   # 없으면 그 위임은 성립하지 않는다
  ```
- ⚠ **`--repo .` 가 이 저장소 이름으로 풀리지 않는다** — TUI 가 `unknown/...` 으로 읽는다.
  **`--repo <소유자>/<저장소>` 를 명시로 준다.**
- ⚠ **CLI 에는 계획 승인 명령이 없다**(`list`·`new`·`pull` 뿐). `Planning` 이 끝나면
  상태가 **빈칸**으로 멈추는데 그것이 승인 대기다 — **승인은 TUI(`jules`)에서 한다.**
- ⚠ **herdr 는 `jules` 를 에이전트로 잡지 않는다**(`agent start --kind` 목록에 없다).
  그 판에는 `agent prompt`·`send-keys` 가 안 듣고 **`herdr pane read <판>` 만 된다** —
  즉 그 판은 **읽기 전용 감시창**이고 위임은 셸의 `jules remote new` 로 한다.
- ⚠ **`jules` 출력을 `head`·`sed -n` 으로 끊지 마라** — node 가 EPIPE 로 죽어
  `Unhandled 'error' event` 를 낸다. **파일로 받아서 읽어라.**

★ **위임 자체의 적합성 판단은 따로 있다** — 「Jules 위임에 적합한 업무 유형」 노트가
정본이며, 요지는 **아키텍처 개편·설계 판단은 넘기지 말고 범위를 한정한 구현·시험 작성만
넘긴다**는 것이다. ⚠ 그리고 **Jules 는 PR 만 올리므로 게이트 전건은 이쪽에서 돌려야 한다**
(클라우드 VM 에는 이 저장소의 `.venv` 가 없고 `uv.lock` 은 `.gitignore` 안이다).
