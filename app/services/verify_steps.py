"""검증 모드 — 렌더러의 10단계를 **사용자의 네 걸음 위에 얹는다**.

## ★★★ 이 파일이 새로 계산하지 않는 이유

중간값을 단계별로 늘어놓는 렌더러는 **이미 있다** —
`core/report/verification.py::render_verification_markdown` 이 10단계를 내고,
...
## ⚠⚠ 단계가 10으로 갈리지 않으면 **멈춘다**

렌더러가 단계를 늘리는 날 화면이 조용히 아홉만 그리면 사용자는 없는 단계를
...
#:
#: ★ **R69/WP-1 이 9 에서 10 으로 올렸다.** 순수 재번호가 아니다 — 옛 1단계
#: 하나가 **새 1「가구 수요」와 새 4「경제성 입력」으로 갈렸고**(검토서 §3.1 ·
#: §5 · §6-2) 옛 4~9 가 5~10 으로 밀렸다. 2·3단계는 번호가 그대로다.
STAGE_COUNT = 10
    ...
    (
        ...
        VerifyGap(
            ...
            reason=(
                ...
                "실제로 쓴 단가와 그 출처·신뢰도는 위 4단계 대장 표에 그대로 "
    ...
    (
        ...
        VerifyGap(
            ...
            reason=(
                ...
                "1년차에만 있으므로(위 5단계 표 5행) 「어느 편익이 몇 년차에 "
                "얼마인가」는 그릴 수 없다. ⚠ 1년차 분해를 20년으로 되짚어 "
                "채우지 않는다 — 운영비는 연 2% 오르고 편익은 평탄이라 되짚은 "
                "표는 실제 장부와 어긋난다. 총액 20연차는 아래 8단계에 그대로 "
                "있다. (착수 순서에 아직 번호가 없는 새 항목)"
            ),
        ),
    ),
...
def _household_count_fill(count: int) -> VerifyFill:
    ...
    입혀 옮긴 것이며, 저장소가 고른 수가 아니라는 사실을 `note` 가 글자로
    적는다.

    ## ⚠⚠ 문면을 고쳤다 — **통로가 둘이 되어 옛 문면이 거짓이 됐다** (R65/WP-2c)

    종전 `note` 는 *「이 수는 **실행 입력이 정했다** — 저장소가 가진 값이
    아니다」* 였다. 그때는 참이었다: 대장의 `load.household.count` 가
    `track: blocked` 라 통로가 실행 입력 하나뿐이었다. R65 에 사용자가 값을
    정해 주어(*「가구수를 20가구로 설정」*) 대장이 답하게 됐고, 그러면 **화면에
    아무것도 적지 않은 실행에서도 이 칸에 20호가 선다** — 그때 옛 문면은
    거짓이다.
    ⚠ **「저장소가 고른 수가 아니다」는 그대로 참이다** — 대장의 그 값도
    사용자가 정해 준 것이며, 그 사실은 항목의 `derivation_method` 가 진다.
    """
    return VerifyFill(
        tag=HOUSEHOLDS_TAG,
        ...
        value=f"{count:,}호",
        note=(
            "이 수는 실행 입력(실행 화면의 「실증단지 규모」 칸)이 정했거나, "
            "적지 않았으면 분석 설정 대장의 `load.household.count` 에서 왔다 — "
            "어느 쪽이든 **사업 계획이 정해 준 값이고 저장소가 고른 수가 "
            "아니다**. 대장은 부하를 「kWh/호·년」 단위로 갖고, 단지 총부하는 "
            "이 수를 곱한 것이다. 아래 모든 수량과 금액이 그 총부하 위에 선다."
        ),
    )


#: 사용자의 네 걸음 ↔ 렌더러 10단계. **문면은 사용자 판정 §1 「결과」 그대로다.**
#:
...
#: 봐서는 알 수 없다.
#:
#: ⚠⚠ ② 「전력공급비용」(6단계 · 1년차 한 벌)과 ④ 「연차별 비용」(8단계 ·
#: 연차 1~20)을 **같은 표로 합치지 않는다.** 합치면 물가·교체·잔존이 1년차
#: 값에 눌려 사라진다 — 앞의 것은 설비의 단가표이고 뒤의 것은 사업자의 20년
#: 장부다.
#:
#: ★★★ **편익 화폐화 단계는 ④ 에 싣는다.** 오케스트레이터가 넘긴 대응표
#: (`.orch/R63/WP-S3.md` ⓑ)는 **그때의 번호로** ② 를 2·3·5, ④ 를 6·7·8·9 로
#: 적었고 **편익 단계가 어느 묶음에도 없었다** — 그대로 따르면 편익 화폐화가
#: 화면에서 통째로 사라진다(검사 `test_all_renderer_stages_are_on_screen` 이
#: 그 상태를 잡았다). 함수를 열어 확인한 결과 그 단계는 편익 5행의 1년차 금액과
#: 그 산식이며, 사용자의 ④ *「분산e사업자의 연차별 비용, **편익**」* 이 그것을
#: 요구한다. ② 는 **공급비용**의 자리이므로 편익이 갈 곳이 아니다.
#: ⇒ ④ = 5 · 7 · 8 · 9 · 10. 이 판단의 경위는 `.orch/R63/result_S3.md` §9.
#: ⚠ 위 대응표의 수는 **승격 전 배열**의 번호다(옛 4·5·6·7·8·9 = 새 5·6·7·8·9·10)
#: — 옛 기록을 고쳐 적지 않고 지금 배열을 이 줄이 갖는다.
#:
#: ★★ **R69/WP-1 — ① 이 두 단계를 든다.** 옛 1단계가 새 1「가구 수요」와
#: 새 4「경제성 입력」으로 갈렸고, 그 둘은 **같은 걸음의 것**이다: ① 의 제목이
#: *「기존 전기사용자 정보 · 가구별 전력소비패턴(계절별) · **전력요금**」* 이라
#: 요금·단가가 이 걸음의 것이고, `progressive_tariff` 빈 칸도 여기 서 있다.
#: ⛔ **새 4 를 ④ 로 내리지 않았다** — ④ 는 「연차별 비용·편익」이고 경제성
#: «입력»은 비용이 아니라 단가다. 화면에서 1 다음에 4 가 오는 것은 이 걸음이
#: 옛 1단계가 실었던 것을 **그대로 다 싣기** 때문이다.
#:
...
_GROUP_PLAN: tuple[tuple[int, str, str, tuple[int, ...]], ...] = (
    ...
    (
        ...
        (
            "← ① 의 1단계가 이 실행이 받은 가구 수요를 싣고 4단계가 분석 설정 "
            "대장에서 읽은 단가·수명·할인율을 싣는다 — 그 둘이 이 걸음의 입력이다. "
            "2단계가 그것으로 자원 목록(용량·운전방식)을 세우고, "
            "3단계 운전이 그 목록을 그대로 받으며, 6단계 운영비가 3단계의 계통 "
            "수전 수량과 4단계의 요금 단가를 함께 쓴다."
        ),
        (2, 3, 6),
    ),
    (
    ...
    (
        ...
        "분산e사업자의 연차별 비용 · 편익",
        (
            "← ② 의 6단계 운영비(1년차)와 ③ 의 순수요(계통 구매량)가 이 걸음의 "
            "연차 장부로 들어간다. 5단계가 ③ 의 운전 수량과 ① 의 단가로 편익을 "
            "화폐로 바꾸고, 7단계가 교체·잔존을 세우고, 8단계가 그 둘을 현금흐름 "
            "행으로 옮기며 1년차 값끼리 대조하고, 9단계가 그 행 전체로 지표를 "
            "내고, 10단계가 지원율 변형을 비교한다. ⚠ ② 의 「전력공급비용」은 물가 "
...
def build_verify_groups(report: CaseReport) -> tuple[VerifyGroup, ...]:
    """검증 모드 화면이 그릴 것 — **네 걸음에 10단계를 얹은 것**.
