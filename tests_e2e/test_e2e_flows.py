...
**「마법사」도 지금은 눌러 넘길 것이 없다.** `web/templates/dashboard.html` 의
`#wizard` 는 네 걸음을 **글자로** 싣고 「다음 단계」 단추는 `type="button"` 이며
아무 데도 가지 않는다. 그래서 아래 시나리오 1 은 마법사의 네 걸음이 화면에 **있는지**
까지만 보고, 실제 진행은 실행 폼으로 한다.
