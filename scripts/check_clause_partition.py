...
실측하면 지금 저장소에 **10건**이 걸린다. 면제 목록이 아니라 **부채 목록**이며
...
아무도 안 본 취약점이다」*), 두 갈래가 섞여 있고 사유에 갈래를 적었다.
    ...
    부채       조항이 리포트 표시를 요구하는데 그 조항의 인용 중 `core.report`
               를 만지는 것이 하나도 없다. **실물 결손이며 이 WP 가 고치지
               않았다** (WP-14 는 `core/` 를 손대지 않는다).
...
KNOWN_GAPS: dict[tuple[str, str], str] = {
    ...
    ("FR-402-AC7", "core.report"): (
        "부채 — 「관점별 편익 집합이 서로 다름을 리포트에 명시하고, 보조금은 "
        "이전지출로 처리」. 인용 1건(tests/cba/test_transfer.py)은 뒷절만 잰다"
    ),
