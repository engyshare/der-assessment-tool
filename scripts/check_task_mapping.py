...
#: 인용 수집. `SC-\d+` 를 함께 받는 이유 —
#:
#: §9 보안 조항은 **표 한 행이 곧 하나의 규범 문장**이라 수용기준 ID 가
#: 요구사항 ID 와 같다(`SC-8`). 08-08 이전에는 SC 가 추적표 밖이라 이 정규식이
#: 볼 것도 없었지만, 표를 읽기 시작한 뒤로는 **작업 목록이 이미 `SC-8` 을
#: 인용하고 있는데도 「미인용」으로 세는** 상태가 된다.
#:
