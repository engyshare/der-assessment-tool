...
#:
#:   구매 단가 상단 400  → *전환* 갈래. 보조 80% 의 결론을 0 선 아래로 크게
#:                        끌어내려 임계값 탐색이 넉넉히 돌게 한다
#:   설비단가 하단 둘    → *회수* 갈래. 무보조의 **동반 하락**이 0 선을 넘게
#:                        한다. 단독으로는 여전히 넘지 못하므로 「결합에서만
#:                        회수된다」는 대조가 유지된다
#:
...
#: 아래 `assert recovering` 이다). R40 의 이동이 여유를 66,838원 깎았다.
#:
#: ⏸ **구매 단가 상단 400 의 원래 사유는 이제 성립하지 않는다** — 종전 이 자리는
#: *「실물 상단 170 으로는 어느 인자도 보조 80% 의 결론을 뒤집지 못한다」* 였는데,
#: 결론축이 두 번 내려가 **실물 대장에서도 뒤집힌다**(R40 실측 · 보조 80%
#: `+1,550,325`원 · `grid_purchase_price` 상단 170 → `−559,849`원 · 임계
#: 156.73원/kWh · `surplus_sale_price` 하단 80 → `−1,127,489`원 · 임계 92.63).
#: **떼지 않았다** — 무엇이 이 넓힘에 기대고 있는지 재지 않았고, 재지 않은 채
#: 떼면 다른 검사가 조용히 0회 순회로 간다. `todo.md` 가 그 확인을 든다.
