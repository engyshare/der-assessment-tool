...
#: 기계 지역 경로 무늬. ⚠ 본문과 대조군이 **같은 것**을 써야 한다 — 둘로
#: 적으면 한쪽만 좁혀도 대조군이 통과한다.
_LOCAL_PATH = r"(?:(?<![A-Za-z])[A-Za-z]:[\\/]|/(?:home|Users|mnt)/)\S*"
...
def test_no_machine_local_path_reaches_the_reviewer() -> None:
    ...
    ## ⚠⚠ **`https://` 를 드라이브 문자로 읽던 오탐을 닫았다** (R65/WP-2c)

    종전 무늬 `[A-Za-z]:[\\/]` 는 **`https://` 의 `s:/` 에도 맞는다.** R65 가
    대장 `load.ev.annual` 의 `source` 에 공표 자료의 URL 을 세우자 이 검사가
    그것을 *「기계 지역 경로」* 로 신고했다 — **URL 은 어느 기계의 경로도
    아니고**, 오히려 재현에 필요한 출처다(이 검사가 지키려는 것과 반대다).
    ⛔ **검사를 느슨하게 한 것이 아니다.** 드라이브 문자는 **한 글자**이므로
    앞에 다른 글자가 붙어 있으면(`http`+`s:`) 애초에 드라이브가 아니다 — 그
    사실을 뒤보기로 적었을 뿐이고, 진짜 경로는 그대로 잡힌다(아래 대조군).
    """
    text = _markdown()
    offenders = re.findall(_LOCAL_PATH, text)
    assert not offenders, f"기계 지역 경로가 리포트에 남았다: {offenders[:3]}"

    # ★ 대조군 — 좁힌 무늬가 **여전히 진짜 경로를 잡는가.** 이것이 없으면 위
    # 뒤보기를 더 넓혀 검사를 무력화해도 아무것도 말해 주지 않는다.
    # ⚠ **경로를 리터럴로 적지 않는다** — 적으면 `scripts/check_disclosure.py`
    # 가 이 파일 자체를 「로컬 절대 경로 유입」으로 신고한다(SC-3 · 실측으로
    # 걸렸다). 조각을 이어 붙이면 잴 무늬는 그대로 서고 소스에는 경로가 남지
    # 않는다.
    colon, back = ":", chr(92)
    planted = " ".join(
        (f"D{colon}/Dev/x", f"/{'home'}/u/y", f"C{colon}{back}Users{back}z")
    )
    assert len(re.findall(_LOCAL_PATH, planted)) == 3, (
        "좁힌 무늬가 진짜 기계 경로를 놓친다"
    )
...
#: ⚠ **R60/WP-4-fix 가 다시 갱신했다** — 자산의 계절 절을 가정으로 채우자 몫
#: 가중 평균 대표일의 **모양**이 바뀌어(여름의 두꺼운 낮 · 겨울의 큰 저녁
#: 봉우리) 낮 시간대 발전과 가구 부하가 겹치는 몫이 늘었다. 계통 역송이 줄어
#: **역송 수량으로 안분되는** `SurplusSale`·`REC` 가 두 행 모두 내려갔다
#: (태양광 41,397 → 40,567원 · ESS 241,533 → 230,150원, 회수 35.3 → 38.4년).
#: 연간 발전·부하 총량은 그대로다 — 움직인 것은 **하루의 모양**이다.
#: ⚠ **R64/WP-4 가 다시 갱신했다** — 러너가 계절 넷의 대표일을 **각각 돌려
#: 계절일수로 가중 합산**하게 됐다(착수 36ⓐ). 겨울의 부족과 여름의 잉여가
#: 평균 하루에서 서로 상쇄되던 것이 풀려 **계통 역송이 늘었고**(연 1,070.73 →
#: 1,144.59kWh), 역송 수량으로 안분되는 `SurplusSale`·`REC` 가 두 행 모두
#: 올라갔다(태양광 40,567 → **42,244원** · ESS 230,150 → **241,978원**,
#: 회수 38.4 → **35.2년**). 연간 발전·부하 총량은 **그대로**이며(대표일 태양광
#: 10.8000kWh · 부하 9.8630kWh) 움직인 것은 **운전**이다.
#: ⚠ **R64/WP-6b 가 다시 갱신했다** — ESS 방전 배분의 배포 기본값이 「부하
#: 추종」이 되면서(사용자 요구 5) 저녁 부하가 큰 시각으로 방전이 몰렸다.
#: 계통 역송 **총량**은 오히려 조금 줄었지만(대표일 3.135868 → 3.130487kWh)
#: **역송 스텝의 자원 구성**이 바뀌어 안분 몫이 갈렸다 — 태양광 42,244 →
#: **42,403원**(회수 불가 그대로) · ESS 241,978 → **241,089원**
#: (회수 35.2 → **35.4년**). 연간 발전·부하 총량은 이번에도 **그대로**다.
#: ⚠ **R64/WP-7 이 다시 갱신했다** — 「AI 가전」이 가전 부하를 하루 안에서
#: 태양광 잉여 시각으로 옮기면서(사용자 요구 2) ⓐ 계통 역송이 줄어
#: **역송 수량으로 안분되는** `SurplusSale`·`REC` 가 두 행 모두 내려가고
#: ⓑ 저녁 봉우리가 낮아져 ESS 의 `PeakShaving`(77,997 → 70,197원)도 줄었다 —
#: 태양광 42,403 → **43,097원**(역송이 준 만큼 저장장치 몫이 더 크게 줄어
#: **안분 비율이 태양광 쪽으로 옮겼다** · 회수 불가 그대로) · ESS 241,089 →
#: **220,550원**(회수 35.4 → **41.5년**). 연간 발전·부하 총량은 이번에도
#: **그대로**이며(대표일 태양광 10.800000kWh · 부하 9.863014kWh) 움직인 것은
#: **하루의 모양**이다.
#: ⚠⚠ **R65/WP-2c 가 다시 갱신했다 — 이번에는 「사업의 규모」가 바뀌었다.**
#: 사용자 요구(*「가구수를 20가구로 설정」*)로 단지가 20호가 되고 설계 변수 셋이
#: 그 배수를 타면서(태양광 3 → 60 kW · ESS 10 → 200 kWh · 정격출력 5 → 100 kW)
#: 이 표의 **모든 칸이 20배 규모로 다시 섰다**: 초기투자 태양광 4,800,000 →
#: **96,000,000원** · ESS 5,000,000 → **100,000,000원**.
#: ⚠ **연 운영비는 각 100,000원 그대로다** — 고정 O&M 은 대장이 「자원 한 대당」
#: 으로 갖는 값이고 단지 규모를 타지 않는다. 그래서 회수 연수가 규모에 비례해
#: 나빠졌다: 태양광 43,097 → **100,566원**(순편익 566원 · 회수 **169611.3년**) ·
#: ESS 220,550 → **4,056,554원**(순편익 3,956,554원 · 회수 41.5 → **25.3년**).
#: ★ **태양광이 「회수 불가」에서 벗어난 것이 개선이 아니다** — 연 순편익이
#: 566원(운영비를 겨우 넘긴 값)이라 회수가 **16만 년**으로 인쇄될 뿐이며, 그
#: 수는 「회수되지 않는다」를 다른 글자로 말한 것이다. ⚠ 그 값을 여기서
#: 고쳐 「회수 불가」로 되돌리지 않는다 — 표가 내는 수를 그대로 붙든다.
#: ⚠⚠⚠ **R66/WP-2 가 ESS 행 하나를 갱신했다** — ESS 초기투자에 **PCS 원/kW 항**이
#: 서면서(사용자 판정 ③) 그 행의 취득가가 100,000,000 → **105,000,000원**이 되고
#: 단순 회수가 25.3 → **26.5년**이 됐다. **편익·순편익은 한 원도 안 움직였다**
#: (4,056,554 / 3,956,554 그대로) — 움직인 것은 **분자가 아니라 분모**다.
#: ⚠ **태양광 행은 그대로다** — PCS 는 ESS 만의 설비다. 즉 이 표는 *「출력에 값을
#: 붙이면 어느 자원의 회수가 나빠지는가」* 를 자원별로 갈라 보여 준다.
#: ⚠⚠⚠⚠ **R66/WP-5 가 두 행을 다 갱신했다 — 이번에는 「분자」다** (사용자 지시 둘).
#: ⓐ 히트펌프 부하가 2,675 → **3,289.0 kWh/호·년**(없던 급탕 501.6 이 서고 냉방이
#: 조사값 712.4 로 갈렸다)이 되어 **계통 역송이 줄었고**, 역송 수량으로 안분되는
#: `SurplusSale`·`REC` 가 두 행 모두 내려갔다. ⓑ 대장에 **동시율 80%** 가 서서
#: ESS 의 `PeakShaving` 이 3,993,600 → **3,424,374원**이 됐다.
#: ⇒ 태양광 100,566 → **24,755원**(순편익 566 → **−75,245원** · 회수 169611.3년 →
#: **회수 불가**) · ESS 4,056,554 → **3,435,024원**(순편익 3,956,554 →
#: **3,335,024원** · 회수 26.5 → **31.5년**).
#: ★ **태양광이 「회수 불가」로 되돌아온 것이 악화의 표시다** — 위 R65 항이 적어
#: 둔 대로 566원은 「운영비를 겨우 넘긴 값」이었고, 역송이 줄자 그 여유가 사라져
#: 연 순편익이 음수가 됐다. 즉 이 표는 *「부하를 늘리면 태양광의 값어치가 어디서
...
#: 지난다」* 고 믿으면 안 된다.
#:
#: ★★★★ **R71/WP-1-fix 가 탐침의 발전량 자체를 갈았다 — 부하가 아니라
#: «여유»가 원인이다.** R71/WP-1 이 전기차 충전손실 10%(`load.ev.charging_loss`)
#: 를 세워 가구 수전량이 늘자, 이 탐침이 위 R67/WP-N1d 에서 되살려 둔 잉여가
#: **스텝 18 한 곳·0.2574 kWh** 라는 극히 얇은 마진이었던 탓에 통째로 먹혀
#: 다시 0 이 됐다(별도 판정 — `tests/golden/test_probe_gap_margin_scenario.py`
#: 가 아니라 이 파일과 같은 탐침을 쓰는 `test_load_shift_wired.py::_report()`
#: 쪽 문제다). ⇒ **오케 판정으로 탐침의 PV 용량을 2배(0.30)로 넉넉히 키워**
#: 앞으로의 부하 증가에도 다시 먹히지 않을 여유를 줬다. 발전량 자체가 바뀌었으므로
#: 태양광·ESS 가 나눠 갖는 잉여 판매·REC 편익의 **크기**가 다시 갈렸다(안분 논리가
#: 재던 것 자체는 그대로 거짓이다 — 자매 검사가 그것을 잰다).
#: ⇒ 태양광 11,375 → **2,635,211원**(순편익 −88,625 → **2,535,211원** · 회수
#: 불가 → **37.9년**) · ESS 3,069,197 → **7,154,260원**(순편익 2,969,197 →
#: **7,054,260원** · 회수 35.4 → **14.9년**).
#: ⚠ **값을 옮겨 적은 것이 아니라 돌려서 얻었다** — 넓힌 탐침의
#: `cost_benefit_section()` 출력을 그대로 읽었다(R71/WP-1-fix 실측).
_ATTRIBUTED_PAYBACK = (
    ("태양광 (옥상 고정형)", "2,635,211원", "2,535,211원", "37.9년"),
    ("에너지저장장치 (신품)", "7,154,260원", "7,054,260원", "14.9년"),
)
