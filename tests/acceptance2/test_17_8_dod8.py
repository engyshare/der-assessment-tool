...
def test_dod8_verification_cases_actually_execute() -> None:
    ...
    for tag, cls in sorted(registry.items()):
        ...
        if result.returncode != 0:
            raise AssertionError(
                f"{tag}: 테스트 파일에서 테스트를 수집할 수 없습니다. "
                f"구문 오류 또는 import 실패일 수 있습니다.\n{result.stdout}\n{result.stderr}"
            )
...
def test_dod8_common_asset_has_all_rc_ca_cases() -> None:
    ...
    if not test_file.is_file():
        raise AssertionError(f"CommonAsset 테스트 파일이 없습니다: {test_file}")
    ...
    if missing:
        raise AssertionError(f"CommonAsset에 누락된 케이스가 있습니다: {', '.join(missing)}")
...
def test_dod8_six_resource_types_have_complete_coverage() -> None:
    ...
    if missing:
        raise AssertionError(f"자원 6종이 모두 등록되지 않았습니다: {', '.join(missing)}")
    ...
    if test_file.is_file():
        ...
        if missing_ca:
            raise AssertionError(f"CommonAsset에 누락된 케이스가 있습니다: {', '.join(missing_ca)}")
    else:
        raise AssertionError(f"CommonAsset 테스트 파일이 없습니다: {test_file}")
