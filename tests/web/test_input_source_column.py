...
#: −11,502,062원(−6,440원)이 됐다(사용자 요구 5 · 같은 파일의 R64/WP-6b 블록).
#: ⚠⚠ **R64/WP-7 이 다시 갱신했다** — 「AI 가전」이 가전 부하를 하루 안에서
#: 태양광 잉여 시각으로 옮기면서 −11,502,062 → −11,625,181원(−123,119원)이
#: 됐다(사용자 요구 2 · 같은 파일의 R64/WP-7 블록). 연간 부하 총량은 한 kWh 도
#: 움직이지 않았고 하루의 모양만 바뀌었다.
#: ⚠⚠ **R65/WP-2c 가 다시 갱신했다 — 이번에는 「사업의 규모」다.** 사용자 요구
#: (*「가구수를 20가구로 설정」*)로 단지가 20호가 되고 설계 변수 셋(태양광 용량·
#: ESS 용량·ESS 정격출력)이 그 배수를 타면서 −11,625,181 → **−323,257,263원**이
#: 됐다(같은 파일의 R65 블록). 하루의 모양이 아니라 **크기**가 바뀐 것이다.
#: **이 파일이 재는 것은 그 수의 크기가 아니라 「입력 출처 열을 켜도 축이
#: 움직이지 않는가」** 이며, 아래 검사가 화면 날값과 리포트 값을 **같은
#: 객체에서** 왔는지로 잰다 — 그 단언은 이 라운드에도 통과했다.
#: ⚠⚠⚠ **R66/WP-2 가 다시 갱신했다 — ESS 초기투자에 「원/kW 항」이 섰다.**
#: 사용자 판정 ③ 에 따라 PCS 250,000원/kW × 100 kW = 25,000,000원이 서고 배터리
#: 단가가 몫 20% 만큼 줄어(500,000 → 400,000원/kWh) 초기투자가 196,000,000 →
#: **201,000,000원**이 됐다 ⇒ −323,257,263 → **−328,257,263원**(−5,000,000 ·
#: 초기투자는 1년차라 현가계수가 1.0 이다). 같은 파일의 R66 블록이 정본이다.
#: ⚠⚠⚠⚠ **R66/WP-5 가 또 갱신했다 — 그리고 이 갱신이 «꺼진 채로 낡아 있었다».**
#: 히트펌프에 급탕 501.6 kWh 가 서고 냉방이 조사값 712.4 로 갈려 한 호 부하가
#: 2,675 → 3,289(+23.0%) 이 됐고, 동시율 80% 가 최대수요를 40 → 34.30 kW 로 줄였다
#: ⇒ −328,257,263 → **−355,028,932원**(히트펌프 −19,367,215 · 동시율 −7,404,454).
#:
#: ★★★ **이 리터럴이 낡은 것을 아무도 몰랐다** — 사용자 판정으로 `tests/web` 297건이
#: 꺼져 있었고(`DER_RUN_WEB_TESTS`) WP-5 가 축을 움직이는 동안 이 시험은 돌지 않았다.
#: R66 이 닫기 직전에 *「인계본이 적은 「켜는 법」이 실제로 되는가」* 를 재려고 켰다가
#: 발견했다. ⇒ **끈 시험은 「안 도는 것」이 아니라 «낡는 것»이다.**
#: ⚠ 웹 라운드가 이것을 켤 때 **결론축 리터럴을 든 시험이 여기 말고 더 있는지 먼저 훑어라.**
#:
#: ⚠⚠⚠⚠⚠ **R68/WP-9 가 또 갱신했다 — 그리고 «같은 함정을 두 번째 밟았다».**
#: R67/WP-N1·N1b 가 부하의 「하루 안 자리」를 고쳐(전기차 충전에 히트펌프 겨울 몫이
#: 씌워지던 것을 끊고 일수 비례로 · 기기마다 하루 안의 형상을 갖게) 축이
#: −355,028,932 → **−360,695,500원**(누적 **−5,666,568**)이 됐고, 그것을 골든에
#: 재채취한 커밋이 `06738f7`(R67/WP-N1c)이다. **그 커밋이 `tests/web` 을 한 줄도
#: 안 만졌다** — 분해와 손계산은 `tests/casegrid/test_perspective_wiring.py` 의
#: `test_operator_perspective_keeps_the_conclusion_axis` ⚠⚠⚠⚠⚠ 절이 갖는다.
#: ★★★ **바로 위 R66 블록이 적은 그 자리를 그대로 다시 밟았다** — 사용자 판정으로
#: `tests/web` 297건이 여전히 꺼져 있고(`DER_RUN_WEB_TESTS`) R67 이 축을 두 번
#: 움직이는 동안 이 시험은 또 돌지 않았다. R68 의 독립 검증자가 스위트를 켜서
#: 찾았다. ⇒ **이것은 「리터럴 하나」가 아니라 «꺼져 있는 한 반복되는 구조»다.**
#: ⚠ 이 수를 손으로 적지 마라 — `DER_RUN_WEB_TESTS=1` 로 이 시험을 돌려 실제 축을
#: 읽고 그 값을 적어라(R68/WP-9 가 그렇게 했다).
#:
#: ⚠⚠⚠⚠⚠⚠ **R69/WP-2 가 또 갱신했다 — 그런데 이번에는 «켜서 잡았다».**
#: 전기요금 인상률(`escalation.electricity_tariff` 연 2.5%)이 파이프라인에
#: 배선되며 요금 연동 흐름 둘(계통 구매 비용 −41,717,709 · 회피 기본요금
#: +8,931,815)이 2~20년차에 함께 올라 축이 −360,695,500 → **−393,481,396원**
#: (**−32,785,896**)이 됐다. 분해와 손계산은
#: `fixtures/golden/scenario_unsubsidized.yaml` 의 **R69/WP-2 블록**이 갖는다.
#: ★ **위 두 블록이 적은 함정을 이번에는 밟지 않았다** — 골든을 재채취한 뒤
#: `DER_RUN_WEB_TESTS=1` 로 `tests/web`·`tests/app` 297건을 돌려 이 리터럴이
#: 낡은 것을 그 자리에서 찾았다(그 절들이 지시한 그대로다). 그러므로 이 수도
#: 손으로 적은 것이 아니라 **그 실행이 읽은 축**이다.
_DEFAULT_NPV = -393_481_396.0
...
def test_the_source_column_does_not_move_the_conclusion(report) -> None:
    ...
    기본값 실행의 무보조 `npv` 는 **−360,695,500원**이며 화면이 인쇄하는 날값이
