...
# ⚠ 이 마커는 **조항과 무관하다. 그래도 유지한다** (R20 판정).
# `FR-201-AC1` 은 *「**GUI** 에서 자원을 추가·삭제·복제하되 엔진 코드는 바뀌지
# 않는다」* 이고, 아래 검사는 **정책 파라미터 미주입 방어**를 본다. 겹치지 않는다.
#
# **그런데 지우면 안 된다.** 이 마커가 `FR-201-AC1`(Must-have·Phase 1)의
# **유일한 매핑**이고 GUI 자체가 아직 없다. 지우면 조항이 미매핑이 되어
# `test_17_9_dod9` 가 빨간불이 되며, 대체 검증도 만들 수 없다 — 검증할 GUI 가 없다.
#
# **`NFR-201`·`NFR-207` 로 옮기지 말 것.** R18 이 분류표의 「플러그인 DER」 행들을
# 그 둘로 재귀속했고 R19 가 둘 다 이미 구현·검증됨을 확인했는데, **그것은
# 분류표 행의 소관 문제였고 이 마커의 소관이 아니다.** 여기서 마커를 옮기면
# 이미 충족된 조항에 매핑이 하나 더 붙고 `FR-201-AC1` 은 그대로 빈다.
#
# **닫는 조건**: GUI 자원 구성 기능을 구현하고, 그것을 보는 테스트에 이 마커를
# 옮긴 뒤 여기서 지운다.
