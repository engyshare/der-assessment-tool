...
class DERContractTests:
    ...
    def test_implements_der_without_engine_knowledge(self) -> None:
        ...
        태그 셋을 리터럴로 알고 있었다. `AC3` 는 이제
        `tests/engine/test_rule_based.py` 가 **엔진을 돌려** 실증한다.

        ⚠ **그 태그 리터럴은 R59 에 없어졌다** — 자원이 `DER.DISPATCH_RULE` 로
        디스패치 규칙을 선언하고 엔진이 그것을 읽는다. **이 검사가 재는 것은
        그대로다**(자원이 엔진을 import 하는가). 자원이 적는 것은 규칙의 **값
        문자열**이지 `DispatchRule` 열거형이 아니며, 그래야 역방향 import 없이
        선언할 수 있다 — 즉 R59 는 이 검사를 우회한 것이 아니라 이 검사가
        허용하는 유일한 형태를 골랐다.
