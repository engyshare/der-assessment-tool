...
SAMPLE: dict[str, object] = {
    ...
    "y_label": "할인율",
    "metric_label": "NPV (만원)",
    # ── R64/WP-5: 계절별 시간대별 운전 (사용자 요구 6) ────────────────────
    #
    # 위 문단이 예고한 그대로 새 차트 하나가 이 사전을 다시 불렀다.
    # **계절마다 같은 스텝 수·같은 자원 목록**이어야 한다 — 갈리면 그 차트가
    # 거부한다(같은 색이 계절마다 다른 자원을 가리키는 그림을 막는다).
    # 일수는 자산이 정하는 값이므로 여기서는 **표본**이며, 배포 경로에서는
    # `CaseReport.seasons[].days` 가 온다.
    "seasons": [
        {
            "name": "봄",
            "days": 92,
