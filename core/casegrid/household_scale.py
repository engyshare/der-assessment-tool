...
⚠⚠⚠ **그래도 골든 3종은 아직 20호로 돌지 못한다 — 거부가 «다른 자리»로
옮겼다** (R65/WP-2b, 미해결). 잉여는 이제 남는데
`core/casegrid/ess_build.py::ESS_POWER_KW = 5.0` 이 **한 호 규모의 정격출력**
이라, 용량만 200 kWh 로 커진 배터리가 하루 방전량을 부하 시각에 다 실을 수
없다 — `ess.power_kw` 로 거부된다(*「정격출력 5kW 까지 채우고도 하루 방전량
33.6068kWh 중 13.6068kWh 가 남습니다」*). 그 상수는 **설계 변수가 아니라 모듈
상수**이고, 그 자리의 주석이 *「용량과 달리 설계 변수로 올리지 않았다 — 이
값이 상한이라 고정해 두어야 용량 스윕이 「용량을 키우면 어디서 출력에
막히는가」를 드러낸다」* 라고 **일부러** 고정해 두었다. ⇒ 그것을 배수에
태울지는 **운전·설계 판정**이라 이 모듈이 정하지 않는다. 진단과 실측은
`.orch/R65/result_2b.md` ②·⑤ 가 갖는다.
