...
## ★★★ R65/WP-2b — **배수가 설비에도 걸렸다.** 이 배수는 부하 전용이 아니다

WP-2 상태에서는 **부하만** 20배가 되고 설비는 1호분이라, 낮에도 발전이 부하를
넘지 못해 태양광 잉여가 0 이 되고 잉여 충전 ESS 가 `DV` 로 실행을 거부했다
(실측: 기기 부하 없이 가구 수만이면 3호부터, 대장의 기기 부하까지 얹으면
2호부터 거부되며, 20호가 성립하려면 태양광이 약 32.8 kW 여야 했다).

⇒ **설계 변수는 「한 호가 갖는 설비」다.** `pv_capacity_kw` base **3 kW** ·
`ess_capacity_kwh` base **10 kWh** 는 한 호 규모이고, 단지 규모를 곱한 것이
「단지가 갖는 설비」다. 그래서 `core/casegrid/e2e_runner.py` 가 그 둘을
`_resolve` 로 얻은 직후 **이 모듈의 `household_scale()` 로 곱한다** — 부하 쪽
(`core/casegrid/seasonal_dispatch.py:789`)과 **같은 함수·같은 배수**다.
⚠ 새 수를 만든 것이 아니다. `_DESIGN_VARS` 의 수는 그대로이고 곱만 붙었다.
⚠ `capex` 는 단가(원/kW)라 **곱하지 않는다** — 총액은 `용량 × 단가` 로 저절로
따라온다(실측: 초기투자가 배수에 정확히 선형이다 · 1호 9,800,000 → 20호
196,000,000 = ×20.000000).

## ★★★ R65/WP-2c — **설비 셋째(ESS 정격출력)까지 같은 배수를 탄다**

WP-2b 뒤에도 골든 3종은 20호로 돌지 못했다 — 잉여 거부는 사라졌는데
**거부가 자리를 옮겼다.** `core/casegrid/ess_build.py::ESS_POWER_KW = 5.0` 이
**한 호 규모의 정격출력**이라, 용량만 200 kWh 로 커진 배터리가 하루 방전량을
부하 시각에 다 실을 수 없었다(*「정격출력 5kW 까지 채우고도 하루 방전량
33.6068kWh 중 13.6068kWh 가 남습니다」* — `ess.power_kw`).

⇒ **그 상수도 「한 호분」이므로 같은 배수를 건다.** 20호면 100 kW 다. 그
자리의 주석이 *「용량과 달리 설계 변수로 올리지 않았다 — 고정해 두어야 용량
스윕이 「용량을 키우면 어디서 출력에 막히는가」를 드러낸다」* 라고 **일부러**
고정을 선언했는데, **그 선언은 무너지지 않는다** — 단지 규모는 설계 선택이
아니라 **규모 환산**이고, 20호 단지 안에서 용량을 스윕하면 여전히 100 kW
상한에 부딪힌다. 고정되는 것은 **호당 출력**이다.

⚠ 통로가 용량 둘과 다르다 — 정격출력은 러너의 모듈 상수가 아니라
`ess_build.py` 의 것이라 `_resolve` 를 지나지 않는다. 그래서 **배수 자체**를
`core/casegrid/seasonal_dispatch.py::_ESSSpec.household_scale_factor` 로 날라
`_case_ess_spec` 이 상수에 곱한다. 그 자리를 고른 사유(러너에서 날라 오면
`seasonal_dispatch.py` 가 NFR-206 코드 상한 500 을 넘긴다)는 그 자리의 ★★★
주석이 갖는다.

⇒ ★ **골든 3종을 20호 구성(60 kW · 200 kWh · 100 kW)으로 다시 뽑았다.**
무보조 `npv` 가 −11,625,181 → **−323,257,263** 이다. 경위와 실측은
`.orch/R65/result_2b.md` ②·④·⑤ 와 `.orch/R65/result_2c.md` 가 갖는다.
