    ...
    for short, resource in (("PV", pv), ("ESS", ess)):
        ...
        for year, cost in sorted(schedule.items()):
            ...
            lines.append(OneOffLine(
                ...
                # 실렸다* 로 읽히는데, **실렸는지는 자원이 정한다** — 실측:
                # `PV` 는 교체비에 계수를 곱하고(`pv.py:563`) `ESS` 는 곱하지
                # 않는다(`ess.py::_acquisitions` 가 `_battery_cost()` 를 그대로
                # 쓴다). 러너가 계수를 넘겨도 후자는 **오늘의 원**으로 남는다.
                # 여기 계수를 적으면 붙임 4 가 ESS 교체비에 대해 **거짓을
                # 인쇄한다** — 이 저장소가 반복해 만난 「문면이 실물보다 후하게
                # 적는다」다. 그 어긋남을 붙드는 것은 검사 쪽이다
                # (`tests/casegrid/test_lifecycle_wiring.py` 의 실효 래칫).
                formula=(
