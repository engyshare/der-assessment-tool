...
class ResourceLine:
    ...
    **옛 제원을 그럴듯하게 계속 인쇄한다.** 아무 예외도 나지 않는다.
    ...
    ## ★ 정책 가정 경고도 **여기로 실려 온다** (`FR-404-AC1` · R48 §7)

    `policy_warnings` 칸은 자원의 `DER.policy_warnings()` 가 낸 문면을 **그대로**
    나른다. 리포트가 그 훅을 직접 부르려면 자원 객체를 들고 있어야 하고, 그러면
    위 「자원을 다시 세우지 않는다」와 같은 함정이 표시 층에 다시 열린다 —
    자원 종류를 알아야 하는 순간 `isinstance(resource, EV_V2G)` 분기가 돌아온다.

    ⚠ **기본값을 두지 않았다.** `DER.policy_warnings()` 독스트링이 적어 둔 대로
    이 훅의 「없음」은 **보수적이지 않다** — 잊으면 제도 근거 없는 편익이 아무
    표시 없이 실리고 아무도 묻지 않는다. 칸을 필수로 두면 `ResourceLine` 을
    새로 세우는 자리마다 **무엇을 실을지 정하지 않고는 지나갈 수 없다.**
    """
    ...
    capacity: str
    #: 「선언 + 배분」을 **합친** 한 문면 — 「전량 판매 (선언) · 본 실행 배분:
    #: 집 우선」. ⛔ **문면을 바꾸지 마라** — 심의 리포트 붙임 6
    #: (`core/report/dispatch_sections.py::dispatch_rule_section`)과 골든이
    #: 이것을 그대로 본다.
    operating_mode: str
    #: 위 합친 칸의 **뒷조각만** — 이 실행이 실제로 적용한 배분(「본 실행 배분:
    #: 집 우선」·「방전 배분: 부하 추종 (방전창 18~21시 안)」).
    #:
    #: ## ★ 왜 칸을 하나 더 두는가 — 문자열을 쪼개지 않기 위해서다
    #:
    #: 검증 3단계는 **선언과 실제를 두 열로** 그려야 한다(R68/WP-2 · 검토서
    #: §3.3 — 「전량 판매」와 「자가소비율 94%」가 한 칸·한 문서에 나란히
    #: 서서 모순으로 읽혔다). 합친 문면을 표시 층에서 ` · ` 로 쪼개 가르면
    #: **조용히 틀린다** — 배분 문면 «안에도» 그 구분자가 있다(위 ESS 예).
    #: ⇒ 러너가 이미 들고 있는 두 조각을 각자의 칸에도 실어 표시 층이
    #: **파싱 없이** 가르게 한다.
    #:
    #: ⚠ **기본값을 두지 않았다** — `policy_warnings` 와 같은 판단이다(아래).
    #: 비워 두면 검증 3단계의 그 칸이 `—`(「이 실행은 배분을 따로 고르지
    #: 않았다」)를 인쇄하는데, 그것은 **빈칸이 아니라 진술**이므로 잊었을 때
    #: 조용히 거짓이 실린다. 필수로 두면 `ResourceLine` 을 세우는 자리마다
    #: 무엇을 실을지 정하지 않고는 지나갈 수 없다.
    applied_allocation: str
    lifetime_years: int
    ...
    produces: tuple[str, ...]
    #: 이 자원이 낸 **정책 가정 경고 문면** — `DER.policy_warnings()` 의 값을
    #: 그대로 나른다(위 독스트링). ⚠ **리포트는 이 문자열을 분류하지 않는다** —
    #: 「제도 필요」와 「제도 보완 필요」의 구분도 **문면 안에** 있어야 하며
    #: (R48 판정 §6), 표시 층이 문자열을 보고 갈래를 치면 문면이 바뀔 때
    #: 조용히 깨진다.
...
class BenefitAttribution:
    """편익 한 갈래의 금액 중 **어느 자원이 얼마를 벌었는가** (R43-E2).

    ## 왜 `BenefitLine.resource_code` 로는 안 되는가 — 그것은 **선언**이다

    `resource_code` 는 *「이 편익 갈래를 누가 만드는가」* 라는 **한 자원의
    이름**이고, 4.3 은 그것을 그대로 금액 귀속으로 썼다
    (`line.tag in resource.produces`). 기준 구성에서 그 귀속이 거짓이었다 —
    잉여 판매 754,820원의 근거 수량은 계통 송전 **18.80kWh** 인데 그중
    **8.00kWh 는 저장장치 방전분**이고(붙임 7 대표일 13~16시), 4.3 은 전액을
    태양광 몫으로 적어 단순 회수를 7.3년으로 인쇄했다.

    한 칸으로는 그것을 담을 수 없다. **한 편익이 여러 자원에서 나오는 것이
    구성의 사실**이므로 자료형이 *「한 갈래 × 한 자원」* 의 행을 갖는다.

    ⚠ **`resource_code` 를 늘려 담지 않았다.** 늘리면 붙임 4 의 표시와 러너의
    조인(`_resource_lines::produced_by`)이 같은 칸을 서로 다른 뜻으로 읽게
    되고, 그 어긋남은 아무 예외도 내지 않는다.

    ⚠ **합계는 원 단위로 보존된다.** 행의 합이 `CaseBasis.annual_benefit_won`
    과 같아야 하며(`NFR-103-M1`), 그것이 이 자료형의 유일한 항등식이다 —
    깨지면 4.3 의 「연 편익」 열이 프로포마가 쓴 편익과 다른 사업을 말한다.
    """

    #: 어느 편익 갈래인가 — `BenefitLine.tag` 와 같은 키.
    tag: str
    #: 그 몫을 가져가는 자원의 **이름** — `ResourceLine.name` 과 같은 키다
    #: (위 「두 규약」의 ①). 붙임 4 의 짧은 코드가 **아니다** — 4.3 이
    #: `ResourceLine` 과 조인하는 자리이므로 코드를 넣으면 빈 교집합이 된다.
    #: **빈 문자열은 「자원에 귀속되지 않는 편익」** 이며 4.3 이 잔여로 싣는다.
    resource_name: str
    #: 이 자원 몫의 1년차 금액(원).
    annual_won: int
    #: **왜 그 금액인가** — 안분 근거 문면(수량과 비율까지). 4.3 이 표 아래에
    #: 성립 조건을 지을 때 이 문면을 그대로 쓴다. 비워 두면 표가 *「1:1 로
    #: 귀속했다」* 를 다시 문장으로 박게 되고, 그것이 이 자료형이 생긴 이유다.
    basis_note: str
...
class CaseBasis:
    ...
    #: 잉여를 **파는** 단가(원/kWh) — 대장 `tariff.surplus_direct_sale`.
    #:
    #: ⚠ **위 구매 단가와 같은 이유로 담는다 — 그리고 그 이유가 여기서 더
    #: 크다.** 자가소비는 잉여판매와 **배타(유형 A)** 라, 켜는 순간 같은
    #: 전력의 판매 수입이 사라진다. 즉 *「자가소비를 반영하면 좋아지는가」* 의
    #: 답은 **두 단가의 차이**이며, 그 차이를 계산하려면 붙임 8 이 파는 단가도
    #: 알아야 한다. 담기 전에는 알 수 없었고, 그래서 그 행이 *「순 방향은 두
    #: 단가의 차이가 정한다」* 에서 멈춘 채 **담당자에게 빼기를 시켰다**
    #: (문의사항 나-5).
    #:
    #: ⚠ **편익 금액에서 되계산하지 않는다.** 잉여판매 금액 ÷ 송전량으로도
    #: 단가가 나오지만, 그것은 송전이 0 인 구성에서 0으로 나눠지고 정산 구조가
    #: 바뀌면 **다른 물리량으로 나눈 그럴듯한 수**가 된다.
    surplus_sale_price_won_per_kwh: float
    #: 평가 대상 자원 — 위 `ResourceLine` 참조.
    resources: tuple[ResourceLine, ...]
    #: 편익 갈래별 금액 — 위 `BenefitLine` 참조.
    benefits: tuple[BenefitLine, ...]
    #: 그 금액의 **자원별 몫** — 위 `BenefitAttribution` 참조. 4.3 이 이것으로
    #: 「연 편익」 열을 짓는다.
    #:
    #: ⚠ **선택 필드로 두지 않았다.** 두면 안 채운 실행에서 4.3 이 옛 귀속
    #: (`line.tag in resource.produces`)으로 조용히 되돌아가고, **빠졌을 때
    #: 나는 증상이 없다** — 표는 그대로 인쇄되고 수만 틀린다. 그것이 이 칸이
    #: 고치러 온 상태 그 자체다(`CaseBasis` 머리말의 「선택 필드로 두지
    #: 않았다」와 같은 근거).
    benefit_attributions: tuple[BenefitAttribution, ...]
