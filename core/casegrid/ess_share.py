...
`core/casegrid/e2e_runner.py` 는 이 모듈을 import 하지 않는다. 어느 케이스도
아직 몫을 쓰지 않으므로 **결론축은 한 원도 움직이지 않는다** —
`tests/casegrid/test_ess_share.py` 의 ⑤ 가 그것을 소스에서 기계로 못 박는다.
몫마다 **어느 편익이 서는지**는 이 모듈이 정하지 않는다(다음 WP 의 몫이다).
여기까지가 *「몫을 수로 만드는 자리」* 다.
"""
