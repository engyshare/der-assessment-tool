...
class ESS(DER):
    ...
    def value_streams(self) -> tuple[str, ...]:
        ...
        **같은 산식이며 시험이 두 값을 대조한다.**
        ...
        ⚠⚠ **그러나 「배타는 실행 경로가 건다」는 지금 거짓이다 (R51/WP-7 실측).**
        R51/WP-3 이 이 자리에 *「배타는 실행 경로(`assert_no_exclusions`/
        `build_report`)가 건다」* 고 적었는데 **둘 다 유형 `E` 를 걸지 않는다**:
        `assert_no_exclusions()` 는 **유형 `A` 만 거부하고**(그 함수 독스트링이
        조항 근거를 적는다), 유형 `E` 를 라벨링하는 `build_report()` 는
        **배포 코드에서 부르는 자리가 0곳**이다. 실측 — 「계통 방전」을 골라
        `NWAs` 단가를 준 실행이 **거부 없이 서고** `NWAs` 와 사업자 운전 편익이
        **함께 계상된다.** R26 이 *「거부 기계는 있는데 배포 코드가 아무도 안
        부른다」* 로 밟은 형태가 **한 층 위에서 되풀이된 것**이다.
        ⚠ **어느 쪽으로 고칠지는 판정이다** — 유형 `E` 는 이 저장소가
        *「「막는」 규칙이 아니라 「성립하지 않음을 선언」하는 규칙」* 으로 세운
        것이라(`docs/exclusion-rules.yaml`) **거부로 바꾸면 그 선언과 어긋난다.**
        `status.md` 「다음에 집을 것」의 **★V** 가 그 자리다.
        """
