...
def perspective_section(report: CaseReport) -> list[str]:
    ...
    lines += [
        ...
        "- **사회 열의 편익 합계가 0원인 이유** — 배선은 서 있다"
        "(`DistributedBenefit` 이 사회 열에 실린다, R53/WP-1). 값이 0인 것은 "
        "`FR-404` 가 기본 비활성(값 0)으로 규정했기 때문이며(대장이 "
        "`benefit.distributed_credit.*` 다섯 칸 — 송배전 회피·손실 감소·"
        "계통서비스·온실가스·회복력 — 을 모두 `track: default0` 으로 둔다), "
        "제도 근거가 확인되지 않았다.",
