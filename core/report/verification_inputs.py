...
def season_lines(report: CaseReport) -> list[str]:
    ...
    head = [
        "",
        # ⚠ 단위가 **표 제목과 열 제목 둘 다**에 선다 (R68/WP-7 · 검토서 §4.2) —
        # 이 표는 «계절 연간값»이고 바로 위 대표일 표는 «kWh/스텝»이라, 같은 파일에
        # 두 축이 서 있다. 제목에 단위가 없으면 읽는 눈이 두 표의 수를 같은 축으로
        # 읽는다. ★ **수를 바꾸지 않았다 — 칸의 문면은 그대로다.**
        "**계절별 운전 — 계절마다 따로 돌린 결과** (사용자 요구 3·6 · 단위 `kWh/년`)",
        "",
    ...
    lines = [
        *head,
        "| 계절 | 일수 (일) | 그 계절의 연간 계통 송전 (kWh/년) "
        "| 연간 계통 수전 (kWh/년) | 하루 안에서 옮긴 가전 부하 (kWh/년) |",
        "|---|---|---|---|---|",
        *(
