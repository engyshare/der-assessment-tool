...
def resource_detail_section(basis: CaseBasis) -> list[str]:
    ...
    lines += [
        ...
        # ★ **「설비 제원은 금액이 아니다」를 지웠다 (R43-H · 문의사항 나-8).**
        #
        # 그 문면은 **거짓이었다.** 이 자리가 가리키는 러너 모듈 상수에는
        # 첨두 기본요금 8,320원/kW·월(첨두 절감 편익 전체를 정하는 단가)과
        # 자원별 고정 운영비 100,000원/년이 함께 있었다. 문의는 그 어긋남을
        # 정확히 짚었다 — *「금액인 이 값에는 들어맞지 않는다」*.
        #
        # 기본요금은 이 라운드에 대장으로 옮겼으므로(`Q-6` ·
        # `tariff.hv_single_contract.demand_charge`) 남은 금액은 고정 운영비
        # 하나다. **그것을 이름으로 적는다** — 「금액이 아니다」로 뭉개면
        # 검토자가 고정 운영비의 출처를 대장에서 찾게 되고 찾지 못한다.
        # ★ **「이용률」을 이 목록에서 내렸다 (R67/WP-N2).** R67/WP-N2 가
        # `PV_CAPACITY_FACTOR` 를 대장(`capacity_factor.pv_rooftop`)으로 옮겼고,
        # 그 뒤로 이 자리가 이용률을 「모듈 상수」라 부르면 **거짓**이다 — 위
        # R43-H 항이 「고정 운영비」를 이름으로 적어 넣은 것과 **같은 이유**이며
        # 방향만 반대다. 남은 것은 정격출력·수명 등 순수 제원과 고정 운영비다.
        "- 그 밖의 제원 소유자 — `core/casegrid/e2e_runner.py` 모듈 상수 "
        "(전제 대장 아님 · 정격출력·수명 등 제원과 위 표의 "
        "**고정 운영비**)",
        "- 대장에서 오는 값 — 설비 단가 · 요금 단가 · 분석기간 "
        "(신뢰도·출처·기준연도는 붙임 1 · 인자별 대장 키는 붙임 2)",
        "",
    ]
    return lines
