...
단계 파일 아홉은 각자의 **ⓒ 다음 단계로 넘긴 값**을 이미 갖고 있었다. 없던
것은 **그 아홉을 한자리에 모아 보여 주는 곳**이고, 그 자리는 목차다(`00-목차.md`
— 단계 파일을 열기 전에 읽는 곳).
...
`8단계` · `2~9단계`). 그러므로 ⓒ 를 고치면 이 칸이 함께 바뀌고, ⓒ 가 아무
단계도 이름 부르지 않으면 **그 사실을 적는다**(`NO_STAGE_NAMED`) — 지어내지
않는다.

⚠ **자기 단계 번호는 세지 않는다.** 「이 표의 키가 2~9단계 전체에서」처럼 자기를
포함해 적은 문면이 있고, 자기가 자기에게 넘긴다는 칸은 뜻이 없다.

⚠⚠ **부정문을 읽지 못한다** — *「1단계가 아니라 4단계가 받는다」* 라고 적힌 ⓒ 는
...
쪼개고 `app/services/verify_steps.py::STAGE_COUNT` 는 **9** 그대로다.
"""
...
#: (그 마디 하나가 4단계 편익과 5단계 운영비에 걸쳐 있고, 쪼개면 검토서가 적은
#: 사슬과 이 표가 다른 모양이 된다).
#:
...
def _prose(block: StageBlock) -> list[str]:
    ...
    ⚠ **칸 안에 표를 넣을 수 없다.** 1단계의 ⓒ 는 문장 하나와 대장 키 표
...
def dependency_chain_lines(blocks: Sequence[StageBlock]) -> list[str]:
    ...
    갈래(`--split-stages` 없이)의 머리말도 그대로다: 그 갈래는 아홉 단계가 한
