...
def _season_annual_table(
    ...
    lines = [
        "#### 계절별 연간 기여 (kWh/년)",
        "",
        # ⚠ 자원 열의 단위는 **표 제목이 진다** — 이 표는 열이 전부 같은
        # 단위이고, 그때 열마다 되풀이하면 표가 읽기 어려워진다는 판정이
        # 이 파일 `LOAD_HEAD` 위 주석에 이미 서 있다(R68/WP-3 · 검토서 §4.2).
        # ⇒ 「일수」만 단위를 받는다 — 그 열만 단위가 다르기 때문이다.
        "| 계절 | 일수 (일) | " + " | ".join(f"`{name}`" for name in names)
        + " | 계통 송전 | 계통 수전 |",
        "|---|---|" + "---|" * (len(names) + 2),
    ]
