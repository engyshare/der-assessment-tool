...
def _assumed_lines(report: CaseReport) -> list[str]:
    ...
    그런데 그 운전은 **가구 부하가 없고 태양광이 24시간 평탄**하다(붙임 8 두
    항목). 그 상태의 표만 실으면 검토자는 *야간 태양광 발전*과 *부하 없는
    단지*를 실물로 읽는다.

    ⚠ **아래 표로 결론을 바꾸지 않는다.** 부하를 편익 계산에 태우면 잉여판매가
    줄어드는데 그 대가인 자가소비 절감은 소매 단가가 없어 계상할 수 없다 —
    한쪽만 반영한 NPV 는 사업에 불리한 쪽으로 틀린다(NSPM 대칭성). 그래서
    싣는 것은 **수량뿐**이며, 화폐화 조건은 붙임 8 이 진다.
    """
