...
**★ R21 에 그 인자는 사라졌다 — 위 예시는 이제 시그니처와 맞지 않는다.**
남겨 두는 것은 이 계약이 무엇을 없애러 왔는지가 그 형태에 있기 때문이다.
지금의 자리는 셋이다: `build_capex_cashflows()`(스킴대로 계산만) ·
`build_baseline_capex_cashflows()`(FR-607-AC3 의 기준선 정의) ·
`core.casegrid.incentive_cases.build_capex_cashflows_for_all_cases()`
(등록된 목록을 순회 — **위층**이다. `core.casegrid` 가 `core.incentive` 보다
상위라 아래에서 부를 수 없다, `NFR-208-AC1`).
