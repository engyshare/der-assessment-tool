...
class DER(ABC):
    ...
    def value_streams(self) -> tuple[str, ...]:
        ...
        **기본이 「없음」인 이유** (v1.1 신설). `RC-LD-B0` 은 *"부하 자원은 편익을
        ...
        추상이 아니라 기본 구현을 주는 것은 방향 때문이다. 비용 메서드의 기본값
        0은 **비용을 지우므로** 위험하지만, 편익의 기본값 「없음」은 편익을
        지우므로 **보수적**이다. 잊으면 경제성이 나빠 보이고, 나빠 보이는 결과는
        검토를 부른다 — 좋아 보이는 결과와 달리.

        ⚠ **기본값이 보수적이라는 근거는 이 메서드의 것이고, 훅마다 다시 따져야
        한다.** `policy_warnings()` 는 같은 형태를 쓰면서 방향이 반대이며, 그래서
        계약 테스트가 재는 것도 다르다 — 아래 그 독스트링을 보라.
    ...
    # ── 정책 가정 경고 (FR-404-AC1 · R48 §7) ────────────────────────

    def policy_warnings(self) -> list[str]:
        """리포트 상단에 표시할 **정책 가정 경고** 문구 목록 (`FR-404-AC1`).

        제도 근거가 아직 확인되지 않은 가정 위에서 편익을 계상하고 있을 때,
        그 사실을 결과와 **함께** 내보내기 위한 훅이다. 값이 아니라 문면이므로
        어느 수치도 움직이지 않는다.

        **왜 계약으로 올렸나** (R48 판정 §7 · spec §16.2 계약 변경 절차).
        v1.1 까지 이 메서드는 `EV_V2G` **한 자원에만** 있었고, 그래서 리포트가
        이것을 부르려면 `isinstance(resource, EV_V2G)` 로 갈래를 쳐야 했다.
        사용자 승인 문면이 *「제도 변경 사항은 향후에도 발생할 수 있는 사항이므로
        본 설계가 해당 사항을 수용할 수 있게 작성되야 함」* 이므로, 갈래를 치면
        **다음 제도마다 또 우회**하게 된다. 훅을 계약에 두면 리포트는 자원의
        종류를 몰라도 되고, 새 자원이 경고를 낼 때 리포트는 한 줄도 바뀌지 않는다.

        ## ⚠⚠ `value_streams()` 와 **형태는 같고 방향은 반대다**

        두 훅 다 「자원별 성실성에 맡기면 반드시 빠진다」는 같은 이유로 계약에
        서고, 기본 구현을 주어 상속으로 강제한다. 그러나 **기본값이 무엇을
        지우는지가 서로 반대다**:

            value_streams()    기본 「없음」은 **편익을 지운다** → 경제성이 나빠
                               보인다 → **보수적**이다. 잊어도 결과가 검토를 부른다
            policy_warnings()  기본 「없음」은 **경고를 지운다** → 제도 근거 없는
                               편익이 **아무 표시 없이** 실린다 → **보수적이지 않다**.
                               잊으면 결과가 그럴듯해 보이고 아무도 묻지 않는다

        그래서 여기서는 *「기본값을 두었으니 안전하다」* 가 성립하지 않는다.
        기본값은 **리포트가 자원 종류를 모르게 하기 위한 것**일 뿐이고, 빠뜨림을
        막는 몫은 기본값이 아니라 **계약 테스트**가 진다 — 그 검사는 값이 무엇인지
        가 아니라 **「자원이 이 훅을 선언했고 `list[str]` 을 내는가」** 를 잰다
        (`DERContractTests.test_policy_warnings_are_declared_as_str_list`).
        재는 것이 그것뿐인 이유도 여기 있다: 어느 자원이 경고를 **내야 하는가**는
        제도 사실이라 계약이 알 수 없고, 계약이 아는 것은 **낼 자리가 있는가**뿐이다.

        ## 무엇을 돌려주는가

        경고가 없으면 **빈 목록**이다. `None` 을 돌려주지 않는다 — 호출측이
        「경고 없음」과 「훅이 없음」을 구분하려 들면 그것이 곧 자원별 분기다.
        문면은 **자원이 소유한다.** 계약은 형식만 고정하고 문장을 쓰지 않는다.
        """
        return []
