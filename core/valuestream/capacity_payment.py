...
class CapacityPayment(ValueStream):
    ...
    ## ★ `payer` 는 **전력시장**이다 — 근사였던 자리를 R50 이 닫았다

    이 자리에는 *「`Payer.GRID_OPERATOR`(「배전사업자」)를 쓰지만 CP 정산은
    전력시장 쪽이므로 이 값은 **근사**이고, 열거를 늘리는 것은 계약 변경이라
    여기서 하지 않는다」* 고 적혀 있었다. **그 근사가 값을 틀리게 두고 있었다** —
    지불 주체는 리포트에 그대로 인쇄되고 관점별 귀속(FR-704)이 그 값을 따르므로,
    시장 정산금이 **배전사업자 부담으로 잡히고** 있었다.

    §16.2 계약 변경 절차대로 `Payer` 에 **`POWER_MARKET = "전력시장"`** 을 세우고
    이 편익만 그리로 옮겼다. **`NWAs` 는 그대로 `GRID_OPERATOR` 다** — 그 편익은
    배전망 증설 회피가 근거라 배전사업자에 가깝고, 다만 *「정산 주체가 확정된
    것은 아니다」* 는 사실이 남아 있어 `core/valuestream/nwa.py` 가 그것을 계속
    적어 둔다. `payer` 를 비우면 활성화 시 `ValidationError`(DV-13)로 거부되므로
    비워 둘 수도 없다 — **가른 뒤에도 그 사정은 같다.**
    """

    tag = "CP"
    #: 생성자의 **등록 용량**으로 계산한다 — 디스패치를 보지 않는다.
    scales_with_dispatch_window = False
    payer = Payer.POWER_MARKET
