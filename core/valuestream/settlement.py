...
class SettlementInputs:
    ...
    **대장 값과 가르는 기준**: 대장은 「조사해서 알 수 있는 값」이고 여기는
    ...
    #:
    #: ⚠ **이 필드만 협상값이 아니다.** 발전량은 설비와 일사가 정하는 물리량이며
    #: 위 기준대로라면 여기 있을 것이 아니다. 그래도 남겨 둔 이유는 조립기를
    #: **자원 없이** 부르는 자리(구조 비교·계약 테스트)가 있어서다 — 필드를
    #: 없애면 그 자리들이 자원을 세워야 하고, 그러면 조립기 검사가 자원 제원의
    #: 변화에 함께 흔들린다.
    #:
    #: **배포 경로에서는 러너가 채운다** —
    #: `core/casegrid/e2e_runner.py::_with_model_generation` 이 `PV` 에게 물어
    #: 넣고, 호출자가 값을 함께 주면 **거부한다**(정본이 둘이 되므로). R32 는
    #: 이 통로가 없어 「집합 PPA」 케이스를 그리드에서 돌릴 수 없었다.
