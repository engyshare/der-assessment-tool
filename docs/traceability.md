...
| `FR-402` | Must-have | 1 | `FR-402-AC1` | 동시 발생 효과는 중복이 아니다 — 지불 주체가 다르거나 물리량이 다르면 정상 계상한다. 중복은 같은 화폐 흐름을 두 번 세는 것으로 한정한다. 시스템은 자가소… | 자동 | test_phase1_dod.py, test_ess_share_benefits.py, test_exclusion_rules_contract.py, test_ess.py, test_exclusion.py, test_exclusion_quantity_axis.py 3건, test_exclusion_reject_wp28b.py, test_quantity_id_reaches_real_benefits.py 3건, test_tou_arbitrage.py |
