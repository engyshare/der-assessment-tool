...
|  |  | 1 | `FR-705-AC2` | 기준선(Without)은 전기사용자의 자가용 설비 처리에 따라 셋으로 갈린다 — ① 자가용 없음(한전 전력 전량) ② 자가용 유지 + 분산에너지 공급(한전 전력… | 자동 | test_pv_allocation.py 3건, test_baseline.py 3건, test_pool_metering_declaration.py 7건, test_baseline_arrangement_wiring.py 4건, test_pool_branch_calculated.py 5건 |
